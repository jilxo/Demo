...
import {senseListSlice} from "./_redux/senseListSlice.tsx";
...
interface SenseListMenuProps {

    sense: SentidoModel;

}

export default function SenseListMenu({sense}: SenseListMenuProps) {
    ...
    const afterCloseDialog = (response: boolean) => {
        if (response) {
            dispatch(senseListSlice.actions.delete(sense.id));
            closeMenu();
        }
