...
import {fontListSlice} from "./_redux/fontListSlice.tsx";
...
interface FontListMenuProps {

    font: FuenteModel;

}

export default function FontListMenu({font}: FontListMenuProps) {
    ...
    const afterCloseDialog = (response: boolean) => {
        if (response) {
            dispatch(fontListSlice.actions.delete(font.id));
            closeMenu();
        }
    ...
    const goToEdit = () => {
        navigate(`${font.id}/edit`, {relative: "route"});
        closeMenu();
    };
