...
import {measuringUnitListSlice} from "./_redux/measuringUnitListSlice.tsx";
...
interface MeasuringUnitListMenuProps {

    measuringUnit: UnidadMedicionModel;

}

export default function MeasuringUnitListMenu({measuringUnit}: MeasuringUnitListMenuProps) {
    ...
    const afterCloseDialog = (response: boolean) => {
        if (response) {
            dispatch(measuringUnitListSlice.actions.delete(measuringUnit.id));
            closeMenu();
        }
