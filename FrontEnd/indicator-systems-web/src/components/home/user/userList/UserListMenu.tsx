...
import {userListSlice} from "./_redux/userListSlice.tsx";
...
export default function UserListMenu({user}: UserListMenuProps) {
    ...
    const afterCloseDialog = (response: boolean) => {
        if (response) {
            dispatch(userListSlice.actions.delete(user.email));
            closeMenu();
        }
    ...
    const goToEdit = () => {
        navigate(`${user.email}/edit`, {relative: "route"});
        closeMenu();
    };
