...
import {actorTypeEditSlice, ActorTypeEditStateModel} from "./_redux/actorTypeEditSlice.tsx";
...
export default function ActorTypeEdit() {
    ...
    const actorTypeFrm = useFormik({
        ...
        onSubmit: (actorTypeModel) => {
            dispatch(actorTypeEditSlice.actions.update(actorTypeModel));
        },
    });

    useEffect(() => {
        if (id)
            dispatch(actorTypeEditSlice.actions.find(id));
    }, []);

    useEffect(() => () => {
        dispatch(actorTypeEditSlice.actions.clean());
    }, []);
    ...
    useEffect(() => {
        ...
        if (isActionOf(result.action, actorTypeEditSlice.actions.findSuccess) && tipoActor)
            actorTypeFrm.setValues(tipoActor);
    }, [result]);
    ...
    return (
        <LateralDialog style={{width: "600px"}}>
            <TitleLateralDialog title="Editar Tipo de Actor"/>
            ...
            <ContentLateralDialog style={styles.content}>
                <TextField
                    ...
                    helperText={
                        actorTypeFrm.errors.nombre || `${actorTypeFrm.values.nombre.length} / 255`
                    }
